...
set N8N_PORT=5678
set SKILL=custom
set CUSTOM_SKILL_PATH=skills/skill_g_first_person_function.md
set SKILL_G_REPO_PATH=skills/skill_g_first_person_function.md
...
rem Use an ASCII wildcard so cmd.exe does not need to parse the Chinese filename.
set "SKILL_G_SOURCE="
for %%F in ("E:\skill\SKILL-G-*.md") do if not defined SKILL_G_SOURCE set "SKILL_G_SOURCE=%%~fF"

if not exist "%SKILL_G_SOURCE%" (
  echo Skill-G source file was not found under E:\skill\SKILL-G-*.md
  echo Put one matching Skill-G markdown file there and run this file again.
  pause
  exit /b 1
)
...
if not defined GITHUB_TOKEN (
  echo GITHUB_TOKEN is required to sync Skill-G to GitHub.
  goto :fail
)
...
set N8N_SKILL_G_PATH=%SKILL_G_SOURCE%

echo Syncing Skill-G to GitHub path %SKILL_G_REPO_PATH%...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $source=$env:SKILL_G_SOURCE; $repoPath=$env:SKILL_G_REPO_PATH; $token=$env:GITHUB_TOKEN; $owner=if($env:GITHUB_OWNER){$env:GITHUB_OWNER}else{'fye2552'}; $repo=if($env:GITHUB_REPO){$env:GITHUB_REPO}else{'video_automation_repo'}; $branch=if($env:GITHUB_BRANCH){$env:GITHUB_BRANCH}else{'main'}; if(!(Test-Path -LiteralPath $source)){throw 'Skill-G source file is missing'}; $headers=@{Authorization=('Bearer '+$token);Accept='application/vnd.github+json';'X-GitHub-Api-Version'='2022-11-28'}; $encoded=[System.Uri]::EscapeDataString($repoPath).Replace(([char]37+'2F'),'/'); $url=('https://api.github.com/repos/'+$owner+'/'+$repo+'/contents/'+$encoded); $existing=$null; try{$existing=Invoke-RestMethod -Method Get -Uri ($url+'?ref='+[System.Uri]::EscapeDataString($branch)) -Headers $headers}catch{if(-not $_.Exception.Response -or [int]$_.Exception.Response.StatusCode -ne 404){throw}}; $body=@{message=('Sync '+$repoPath);content=[Convert]::ToBase64String([System.IO.File]::ReadAllBytes($source));branch=$branch}; if($existing -and $existing.sha){$body.sha=$existing.sha}; Invoke-RestMethod -Method Put -Uri $url -Headers $headers -ContentType 'application/json' -Body ($body|ConvertTo-Json -Compress) | Out-Null; Write-Host 'Skill-G synced.'"
if errorlevel 1 goto :fail

echo Starting local npm n8n for Skill-G...
start "n8n-video-skill-g" cmd /k n8n start
...
start "moyin-worker-watcher-skill-g" cmd /k python workers\worker_watcher.py ^
...
del run.lock
echo Skill-G batch submission finished.
pause
exit /b 0

:fail
echo Skill-G batch failed. If no submission is running, delete run.lock manually.
pause
exit /b 1
