...
if not exist SKILL-E.md (
  echo SKILL-E.md was not found in %cd%.
  echo Put the Skill E source file at E:\n8n-video-gemini\SKILL-E.md and run this file again.
  pause
  exit /b 1
)
...
set N8N_SKILL_E_PATH=E:/n8n-video-gemini/SKILL-E.md
set NODE_FUNCTION_ALLOW_BUILTIN=*

echo Starting local npm n8n for Skill E...
start "n8n-video-skill-e" cmd /k n8n start
...
start "moyin-worker-watcher-skill-e" cmd /k python workers\worker_watcher.py ^
...
del run.lock
echo Skill E batch submission finished.
pause
exit /b 0

:fail
echo Skill E batch failed. If no submission is running, delete run.lock manually.
pause
exit /b 1
