---
name: pain-conflict-fast-cut-generator
description: Generate 20-second pain-conflict fast-cut UGC/Omini ad videos from a Skill A product reference pack. Use after product-reference-standardizer has produced product_profile.yaml and images/product_white_bg.jpeg, when the user wants Skill C: realistic wrong behavior or pain conflict, quick product solution, three fast-cut usage scenes, short CTA, two 10-second Moyin/Omini segment submissions per video, GitHub upload of campaign artifacts, MP4 downloads to E:\视频\<sku_id>_C_<batch_id>, and prompt history saved back to product_reference_pack/<sku_id>/history/.
---

## Environment Config (.env)

```env
VIDEO_PROVIDER=moyinai
MOYIN_API_BASE_URL=https://demo.moyinai.com/api/v1
MOYIN_API_KEY=<SECRET_PLACEHOLDER>
MOYIN_VIDEO_MODEL=veo-omni-flash
MOYIN_VIDEO_SIZE=720x1280
MOYIN_POLL_INTERVAL_SECONDS=8
MOYIN_TIMEOUT_SECONDS=900
TEXT_PROVIDER=mikoto_openai
TEXT_API_BASE_URL=https://api.mikoto.vip/v1
TEXT_API_KEY=<SECRET_PLACEHOLDER>
TEXT_MODEL=fill_text_model_here
IMAGE_PROVIDER=moyinai
IMAGE_API_BASE_URL=https://demo.moyinai.com/api/v1
IMAGE_API_KEY=<SECRET_PLACEHOLDER>
IMAGE_MODEL=gpt-image2
GITHUB_TOKEN=<SECRET_PLACEHOLDER>
GITHUB_REPO_OWNER=fye2552
GITHUB_REPO_NAME=moyin-images
GITHUB_BRANCH=main
CDN_BASE=https://cdn.jsdelivr.net/gh/fye2552/moyin-images@main/
```

---

# Pain Conflict Fast-Cut Video Generator

Use this as Skill C in the product-video pipeline. Skill A creates standardized product references. Skill C creates 20-second pain-conflict fast-cut ads and executes video generation as two 10-second segments per complete video.

Skill C references Skill B's execution layer: read Skill A output, generate prompt payloads, upload to GitHub, submit Moyin/Omini tasks, poll, download MP4s, and save prompt history. Its creative direction is different: real wrong behavior / pain conflict -> product quick solution -> multi-scene fast cuts -> CTA.
...
- `prompts/conflict_hooks_prompt.md`: generate real wrong-behavior openings.
...
```text
campaign_output/<sku_id>_skill_c/asset_manifest.generated.json
```
...
```json
{
  ...
  "required_assets": {
    "white_bg": "images/product_white_bg.jpeg"
  },
  "optional_assets": {
...
```text
campaign_output/<sku_id>_skill_c/
...
Skill C is not a classic creator seeding video.

Core story:

```text
real wrong behavior / pain conflict
↓
...
Show a real-life wrong, inefficient, messy, uncomfortable, or inconvenient behavior.

Focus:

- real everyday scene;
- wrong or inefficient behavior;
- local close-up detail;
- pain point understood quickly.

Do not:

- start with the product;
- start with a perfect result;
- use slow setup;
- exaggerate beyond real life.
...
```text
Create a 10-second vertical TikTok-style ad video.
Timeline:
0-3s: Show a real-life wrong or inefficient behavior using close-up details to amplify the pain point.
3-6s: Quickly introduce the product as the direct solution and show the issue being fixed immediately.
...
Keep the pacing fast, clear, and product-focused.
```
...
Save product-specific pain points:
...
These are not traditional hooks. They are real wrong behavior + detail amplification openings:
...
conflict_hooks:
  - conflict_id: "conflict_001"
    ...
    wrong_behavior: ""
    amplified_detail: ""
    conflict_type: "real_life_wrong_behavior"
```

### `strategy/scene_sequences.yaml`

Save fixed 20-second structure:
...
Save script/scene logic:

```yaml
scripts:
  - script_id: "script_c_001"
    duration: 20
    style: "pain_conflict_fast_cut"
    pain_point: ""
    conflict_hook: ""
    product_solution: ""
    timeline:
...
Skill C does not do influencer dedupe.
...
```text
product_reference_pack/<sku_id>/history/prompt_payloads_C_batch_*.json
```
...
```text
campaign_output/<sku_id>_skill_c/prompts/prompt_payloads.json
```
      ...
      "segments": [
        {
          ...
          "timeline": ["0-3s: pain conflict", "3-6s: product solution", "6-10s: fast-cut scene 1"],
...
### Phase 1: Reference Images (upload first)
...
```text
campaign_output/<sku_id>_skill_c/github/github_upload_log.json
```

The upload log must list every file uploaded with its CDN URL. Video API image references in prompts must be public HTTPS URLs (CDN), never local file paths.
...
Skill C submits by segment, not by full 20-second video.
...
```text
E:\视频\<sku_id>_C_<batch_id>\
```

Example:

```text
E:\视频\宠物毯子_C_batch_001\
├─ video_001_part_01.mp4
├─ video_001_part_02.mp4
├─ video_002_part_01.mp4
├─ video_002_part_02.mp4
└─ polling_log.json
```
...
```text
product_reference_pack/<sku_id>/history/prompt_payloads_C_<batch_id>.json
```
...
```text
E:\视频\<sku_id>_C_batch_001\
```

If it exists, increment until an unused folder is found.
...
8. Dedupe against `history/prompt_payloads_C_batch_*.json`.
...
12. Write task metadata to `E:\marvis\skill\tasks.json`, launch independent cmd window to poll & download MP4s locally (see Deferred Polling). Do not block main session.
